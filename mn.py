...
class Window(QMainWindow):
    ...
    def create_table(self):
        self.cur.execute(
            'create table if not exists Kirish(Password varchar(50), Login varchar(50))')
        self.mydb.commit()
...
class Check(QWidget):
    ...
    def On(self):
        ...
        pas = window.SIGn.new_pas
        lgo = window.SIGn.new_log
        window.cur.execute("select password from kirish")
        self.parol = window.cur.fetchall()
        self.parol = str(self.parol)
        self.paw = str()  # Bazadagi parol
        ...
        if text == self.paw:
            window.cur.execute("update Kirish set (%s, %s)", (pas, lgo))
            miniwindow = QMessageBox(self)
            miniwindow.setText("Muvaffaqiyatli o'zgartirldi")
            miniwindow.setText("✅✅✅✅")
